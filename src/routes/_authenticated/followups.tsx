...
function FollowupsPage() {
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = { all: rows.length, pending_review: 0, sent: 0, failed: 0, cancelled: 0 };
            ...
            <div key={f.id} className="rounded-2xl bg-card border border-border/60 p-5">
              ...
              {f.last_error && (
                <div className="mt-3 text-xs text-[oklch(0.62_0.22_27)] bg-[oklch(0.62_0.22_27)]/8 rounded-lg px-3 py-2 border border-[oklch(0.62_0.22_27)]/20">
                  ...
                  {(f.retry_count ?? 0) > 0 && <span className="ml-2 opacity-70">· {f.retry_count} attempt{f.retry_count === 1 ? "" : "s"}</span>}
                </div>
              )}
              <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <>
                    <button
                      onClick={async () => {
                        ...
                        try { await send({ data: { followupId: f.id } }); toast.success("Sent", { id: t }); invalidate(); }
                        catch (e) { toast.error((e as Error).message, { id: t }); invalidate(); }
                      }}
                      className="cta-glossy rounded-full px-4 py-2 text-sm"
                    >Send now</button>
                    <button
                      onClick={async () => { await cancel({ data: { followupId: f.id } }); invalidate(); }}
                ...
                {f.status === "failed" && (
                  <button
                    onClick={async () => { await retry({ data: { followupId: f.id } }); toast.success("Re-queued"); invalidate(); }}
                    className="rounded-full border border-border px-4 py-2 text-sm hover:bg-secondary"
                  >Retry</button>
                )}
                {f.sent_at && (
                  <span className="text-xs text-foreground/50 self-center">Sent {new Date(f.sent_at).toLocaleString()}</span>
                )}
              </div>
            </div>
          );
...
function StatusBadge({ status, scheduled }: { status: string; scheduled: string | null }) {
  ...
  const tint =
    ...
    status === "failed" ? "bg-[oklch(0.62_0.22_27)] text-white" :
