...
function AuditPage() {
  ...
  return (
    <div>
      ...
      <p className="text-foreground/60 mt-1">Every autonomous action EXECUTOR has taken. Last 200 entries.</p>

      <div className="mt-8 rounded-2xl border border-border/60 bg-card divide-y divide-border/60 overflow-hidden">
        {rows.length === 0 && <div className="p-12 text-center text-foreground/50">Quiet so far.</div>}
        {rows.map(r => (
          <div key={r.id} className="p-4 flex items-start gap-3">
            <div className="mt-0.5">{icons[(r.status as keyof typeof icons)] ?? icons.info}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 text-sm">
                <span className="font-medium uppercase tracking-wider text-xs text-foreground/70">{r.event_type.replace("_", " ")}</span>
