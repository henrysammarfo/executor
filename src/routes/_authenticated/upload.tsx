  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="text-foreground/60 mt-1">
        Drop a transcript or audio. EXECUTOR will do the rest.
      </p>
      ...
      <div className="mt-8 flex gap-2 text-sm">
        ...
        >
          <FileText className="h-4 w-4 inline mr-1" /> Paste transcript
        </button>
        <button
        ...
        >
          <FileAudio className="h-4 w-4 inline mr-1" /> Upload audio
        </button>
      </div>

      <form
        onSubmit={submit}
        className="mt-6 space-y-4 bg-card border border-border/60 rounded-3xl p-6 shadow-soft"
      >
        <input
          required
          value={title}
        ...
        {mode === "text" ? (
          ...
          <label className="block border border-dashed border-border rounded-xl p-8 text-center cursor-pointer hover:bg-secondary/40">
            ...
            <div className="mt-3 text-sm">
              {audio ? (
                <>
                  <b>{audio.name}</b> · {(audio.size / 1024 / 1024).toFixed(1)} MB
                </>
              ) : (
